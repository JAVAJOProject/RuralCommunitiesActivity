...
export default function EventApplyContents({
  labelTexts,
  data,
  eventType,
  ...
  const { postId, title, recruitDate, date, addr, detail, buttons } =
    labelTexts;
  const {
    eventId,
    eventName,
  ...
  return (
    ...
    >
      ...
      />
      {+eventType === 4 && (
        <InputTextAreaBox inputInfo={detail} isRequired={+eventType === 4} />
      )}
      <InputBtnBox
        inputInfo={buttons}
        formRef={formRef}
