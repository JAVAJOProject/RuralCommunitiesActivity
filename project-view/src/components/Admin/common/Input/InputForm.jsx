...
export default function InputForm({inputFormData,isInputRef,change,isCheckList}) {
    ...
    return (
        <>        
            ...
            {inputFormData.map((inputFormItem, index) => (
                <label htmlFor={inputFormItem.key} 
                className={inputFormItem.name === 'u_gender' ? 
                'inputGenderA' : inputFormItem.name === 'u_marketing' ?
                'inputMarketingA' : 'inputLabelA'}
                
                key={inputFormItem.key}  >
                    <p>{inputFormItem.title}</p>
                <input className={'inputDefaultA'} 
