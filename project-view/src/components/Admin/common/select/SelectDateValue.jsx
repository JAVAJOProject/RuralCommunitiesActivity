...
export default function SelectDateValue({selectType, sido, values, setValues, sigungu,setSigungu}) {
    ...
    return (
        ...
        {selectType === 'sigungu' ? 
        (sigungu.map((item) => 
            <option key={`${selectType}_${item.value}`} value={item.value}>{item.title}</option>)) 
        : 
        (values.map((item) => 
            <option key={`${selectType}_${item.value}`} value={item.value}>{item.title}</option>))}
        </>
