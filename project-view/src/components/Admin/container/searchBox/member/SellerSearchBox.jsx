  ...
  const InputRegDateMember = [
    {
      ...
      type: 'date',
      name: 'startDate',
      value: SellerSearchDTO.startDate,
    },
    {
      key: 'endDate',
      type: 'date',
      name: 'endDate',
      value: SellerSearchDTO.endDate,
    },
  ];
  //input id
  const InputIdMember = [
  ...
  const fetchData = async () => {
    try {
      const requestData = await fetchDataPOSTAndObj(
        `/admin/membermanage/selectMember/${requestPageForSellerList}`,
        SellerSearchDTO
      );
      ...
      <TopSearchMiniBox categoryName={titleForSellerList}>
        <FormProvider {...methods}>
          ...
          >
            <InputLabel
              labelType="search"
              inputLabelData={categoryForSellerList}
            />
            <Input
              inputData={InputRegDateMember}
              change={handleOnChangeForSellerList}
            />
            <br />
            <Input
              inputData={InputIdMember}
              change={handleOnChangeForSellerList}
            />
