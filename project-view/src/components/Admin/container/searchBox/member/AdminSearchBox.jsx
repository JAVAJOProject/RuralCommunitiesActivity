  ...
  const InputRegDateMember = [
    {
      ...
      type: 'date',
      name: 'startDate',
      value: adminListSearchDTO.startDate,
    },
    {
      key: 'endDate',
      type: 'date',
      name: 'endDate',
      value: adminListSearchDTO.endDate,
    },
  ];
  //input id
  const InputIdMember = [
    {
      ...
      type: 'number',
      name: 'id',
      value: adminListSearchDTO.keyword,
      placeholder: '아이디 입력',
    },
  ];
  ...
  const fetchData = async () => {
    try {
      const requestData = await fetchDataPOSTAndObj(
        `/admin/membermanage/selectAdmin/${adminListrequestPage}`,
        adminListSearchDTO
      );
      ...
      if (requestData !== undefined) {
        ...
        const [perPostPageNo, totalPostNo] = await fetchDataPOSTAndObj(
          `/admin/membermanage/selectAdminCount`,
          adminListSearchDTO
        );
      ...
      <TopSearchMiniBox categoryName={titleForAdminList}>
        <FormProvider {...methods}>
          <form
            ...
            className="MemberSearchForm"
          >
            <InputLabel
              labelType="search"
              inputLabelData={categoryForAdminList}
            />
            <Input inputData={InputRegDateMember} change={handleOnChange} />
            <br />
