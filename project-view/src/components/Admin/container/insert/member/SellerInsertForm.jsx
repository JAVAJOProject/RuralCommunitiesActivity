...
export default function SellerInsertForm() {
  ...
  const FetchInsertData = async () => {
    try {
      const insertData = await fetchDataPOSTAndObj(
        '/admin/membermanage/registrationMember',
        sellerInsertDTO
      );
  ...
  const onSubmitForMemberInsert = (event) => {
    ...
    const inputValues = formRefForSellerInsert.current.map(
  ...
  const rednderPopup = popupView.current ? (
    <PopupInsert name={sellerInsertDTO.u_name} result={resultMsg} />
  ) : null;
  return (
    <>
      <FormBox clipColor="green" formName="제공자 정보등록">
        <form onSubmit={onSubmitForMemberInsert}>
          <InputForm
            inputFormData={inputsForMemberInsert}
            isInputRef={formRefForSellerInsert}
            change={checkItem}
            isCheckList={isCheck}
          />
