...
export default function MemberInsertForm() {
    const formRefForMemberInsert = useRef([]);


    const [ memberInsertInfo, updateMemberInsertInfo] = useImmer({});
    const [ memberInsertDTO, setMemberInsertDTO] = useState({
        u_name: '',
        u_gender: 'M',
        u_nick: '',
        u_birth: '',
 ...
 const FetchInsertData  = async () => {
    try {
        const insertData = await fetchDataPOSTAndObj(
            '/admin/membermanage/registrationMember', memberInsertDTO 
        );
    ...
    const onSubmitForMemberInsert = (event) => {
        ...
        const inputValues = formRefForMemberInsert.current.map((inputRef) => inputRef.value);
        console.log(inputValues);

        setMemberInsertDTO({
            u_name: inputValues[2],
            u_gender: isCheck.has('M') ? inputValues[1] : (isCheck.has('F') ? inputValues[0] : inputValues[1]),
...
const checkItem = ( value, ischecked) => {
    ...
    const insertLinkForMember = '/admin/user/experiencer/registration'
        
    const rednderPopup = popupView ? <PopupInsert showModal={popupView} setShowModal={setPopupView} name={memberInsertDTO.u_name} result={resultMsg} link={insertLinkForMember}/> : null;
    return (
        <>
        <FormBox clipColor='green' formName='체험자 정보등록'>
            <form onSubmit={onSubmitForMemberInsert}>
            <InputForm inputFormData={inputsForMemberInsert} isInputRef={formRefForMemberInsert} change={checkItem} isCheckList={isCheck}/>
