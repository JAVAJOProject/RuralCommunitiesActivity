...
export default function SellerInsertForm() {
    const formRefForSellerInsert = useRef([]);
    const popupView = useRef(false);


    const [ sellerInsertInfo, updateSellerInsertInfo] = useImmer({});
    const [ sellerInsertDTO, setSellerInsertDTO] = useState({
        u_name: '',
        u_gender: 'M',
        u_nick: '',
        u_birth: '',
 ...
 const FetchInsertData  = async () => {
    try {
        const insertData = await fetchDataPOSTAndObj(
            '/admin/membermanage/registrationMember', sellerInsertDTO 
        );
    ...
    useEffect(() => {
        ...
        const onSubmitForMemberInsert = (event) => {
            ...
            const inputValues = formRefForSellerInsert.current.map((inputRef) => inputRef.value);
...
const checkItem = ( value, ischecked) => {
    ...
    const rednderPopup = popupView.current ? <PopupInsert  name={sellerInsertDTO.u_name} result={resultMsg} /> : null;
    return (
        <>
        <FormBox clipColor='green' formName='제공자 정보등록'>
            <form onSubmit={onSubmitForMemberInsert}>
            <InputForm inputFormData={inputsForMemberInsert} isInputRef={formRefForSellerInsert} change={checkItem} isCheckList={isCheck}/>
